...
/* Test hook: true iff q8_amax's dispatch, under the currently selected
   kernel, resolves to the AVX2 body specifically - pointer identity,
   not a value comparison. See its definition (ops_quant.c) for why. */
int q8_amax_picked_avx2(void);

/* Test hook: true iff THIS translation unit saw LZ_HAVE_Q8R_AVX2 when it
   compiled, i.e. iff ops_quant.c's #include "ops_avx2.h" reached it.
   lz_quantize_q8's tier==3 arm is compiled out silently when it does
   not, and the tiers owe each other bit-identity, so a value comparison
   cannot tell the AVX2 kernel from the SSE2 fallback. The gdn twin
   (lz_q8round32_avx2_compiled_in_gdn) answers the same question for
   ops_gdn.c; the two TUs include the header independently and either
   can go missing alone. */
int lz_q8round32_avx2_compiled_in_quant(void);

/* Counts group executions of lz_quantize_q8's AVX2 arm. The compiled_in
   hook above answers "is the arm in this build"; this answers "did it
   run", which no output comparison can, because the tiers are
   bit-identical by contract. Defined in ops_quant.c unconditionally. */
extern long lz_debug_q8r_avx2_quant;
